...
/**
 * <p>
 * The CSRFTokenManager is in charge of generating and validating CSRF tokens.
 * This CSRF defense allows applications to generate
 * <a href="http://www.corej2eepatterns.com/Design/PresoDesign.htm">
 * synchronizer tokens</a> as recommended by <a href=
 * "https://www.owasp.org/index.php/Cross-Site_Request_Forgery_(CSRF)_Prevention_Cheat_Sheet">
 * OWASP</a>. These tokens are semi-stateless (tied to session, but not stored
 * anywhere) and so can be generated wherever in the application and validated
 * wherever.
 * </p>
 * <p>
 ...
 * </p>
 * <ol>
 * <li>Given a SessionID (must be at least 16 bytes long), first generate a
 * TokenID from a {@linkplain SecureRandom}</li>
 * <li>Next, get the current timestamp</li>
 * <li>Create the text to be encrypted, cryptText, as SessionID|timestamp</li>
 * <li>Use the TokenID as an IV and the first 16 bytes of the SessionID as key
 * to encrypt the cryptText</li>
 ...
 * </p>
 * <ol>
 * <li>Given a SessionID and a Token, first split the token into TokenID and
 * encrypted text</li>
 * <li>Next, decrypt the encrypted text using the first 16 bytes of the
 * SessionID as key and TokenID as IV</li>
 * <li>Split the resulting cryptText into SessionID and timestamp</li>
 * <li>validate that the full SessionID matches the decrypted version (not just
 * first 16 bytes)</li>
 * <li>validate that the timestamp is within the expiration time from now</li>
 * </ol>
 * <p>
 * The generation and validation methods also allow application developers to
 * specify further items to be encrypted (in addition to the sessionID and
 * timestamp). These items are validated in their supplied order, so developers
 * should take care to maintain the same order during validation as was used in
 * generation.
 * </p>
 * <p>
 * <b>Note:</b> By default, a {@linkplain ICSRFErrorHandler} is assigned to the
 * Manager. This handler writes all messages to SysErr (or throws exceptions if
 * an exception occurs) during generation and validation when an error has
 * occurred or validation has failed. Implementors should override this behavior
 * to log CSRF token validation errors to log correlation/SIEM or some metrics
 * system to track potentially malicious activity
 * </p>
 *
 * @author Chris Smith
 */
public class StatelessCSRFTokenManager {
	...
	/*
	 * size of the generated token's ID (not the final token). it must be 16 bytes
	 * long at least
	 */
	...
	/*
	 * manages all error conditions. Default is to write to syserr/throw an
	...
	/**
	 * Create a new {@linkplain StatelessCSRFTokenManager} with all defaults and use
	 * the provided {@linkplain SecureRandom}
	 * 
	 * @param random a {@linkplain SecureRandom} instance to use in generating
	 *               random tokens or null which will generate a new
	...
	/**
	 * Create a new {@linkplain StatelessCSRFTokenManager} with all defaults and use
	 * the provided {@linkplain SecureRandom} and {@linkplain ICSRFErrorHandler}
	 * 
	 * @param random  a {@linkplain SecureRandom} instance to use in generating
	 *                random tokens or null which will generate a new
	 *                {@linkplain SecureRandom}
	 * @param handler a {@linkplain ICSRFErrorHandler} instance to handle reporting
	 *                issues that are raised during processing, or null which will
	 *                default to the {@linkplain DefaultCSRFErrorHandler} instead
	 */
	...
	/**
	 * Configure a new expiration time on tokens. This takes effect immediately on
	 * all outstanding tokens. e.g. if the old expiry were 10 mins and the new
	 * expiry is 20 mins, all tokens generated 19 mins ago are now valid, even
	...
	/**
	 * Builds a secure token used to protect against CSRF attacks. Additional data
	 * may be used to further lengthen the resulting token
	 *
	 * @param sessionID the sessionID of this request
	 * @param otherData (Optional) any other strings that should be used to generate
	 *                  the token. See class definition.
	...
	/**
	 * Generate a key based on a random id, session id, and current time with
	 * optional other data added
	 *
	 * @param id          a random id
	 * @param sessionID   the session of the current request
	 * @param dataToCrypt (Optional) any other strings that should be used to
	 *                    generate the token. See class definition.
	...
	private byte[] generateTokenInternal(byte[] id, String sessionID, String... dataToCrypt) {
		...
		// if other data supplied, append it to the sbCryptText in the same format,
	...
	/**
	 * Ensures that the supplied token is a valid csrf token. Valid tokens are made
	 * up of the current session, randomly generated token, timestamp, and any other
	 * data supplied. The timestamp must be within some number of milliseconds
	 ...
	 * @param otherData (Optional) any other strings that should be used to validate
	 *                  the token. See class definition.
	...
	/**
	 * Tests the given token id + string for validity. Also does internal checking
	 * of string to attempt to detect tampering
	 *
	 * @param tokenId     the random ID to use in key generation
	 * @param sessionID   the session of the current request
	 * @param dataToCrypt (Optional) any other strings that should be used to
	 *                    validate the token. See class definition.
	...
	private boolean validateTokenInternal(String token, String sessionID, String... dataToCrypt) {
		...
		try {
			...
			if (decryptParts.length == (2 + cryptlen)) {
				...
				/*
				 * verify sessions match verify that the timestamp in the token is within the
				 * permitted time allowance and verify all other possible data matches in order
				 */
				if (!decryptedSession.equals(sessionID)) {
					...
					String error = new StringBuilder().append("CSRF Token has expired. Expected: ").append(timestamp)
							.append(" but received: ").append(decryptedTimestamp).toString();
			...
			String error = new StringBuilder().append("Could not validate token ").append(token).append(" for session ")
	 ...
	 * @param textBytes the encrypted value to be decrypted OR the plaintext to be
	 *                  encrypted
	 * @param mode      either {@link Cipher#ENCRYPT_MODE} or
	 *                  {@link Cipher#DECRYPT_MODE}
	 * @return the encrypted or decrypted value, depending on the given mode
	 * @throws NoSuchAlgorithmException           if the underlying code throws this
	 *                                            exception
	 * @throws NoSuchPaddingException             if the underlying code throws this
	 *                                            exception
	 * @throws InvalidKeyException                if the underlying code throws this
	 *                                            exception
	 * @throws InvalidAlgorithmParameterException if the underlying code throws this
	 *                                            exception
	 * @throws IllegalBlockSizeException          if the underlying code throws this
	 *                                            exception
	 * @throws BadPaddingException                if the underlying code throws this
	 *                                            exception
	 * @throws UnsupportedEncodingException       if the underlying code throws this
	 *                                            exception
	 */
	...
	/**
	 * Return the current time for this application, in milliseconds. This method is
	 * exposed to subclasses as system clocks may vary in multinode cloud
	 * environments that may span normal datetimes.
	 * 
